...
import { uploadAvatar } from "@/store/api";
import { actions as api } from "@/store/api";

export default function ModalUser({ open, onClose }) {
  const ServerFileInput = useRef(null);
  const [ServerFileURL, setServerFileURL] = useState("");
  const [CreateServerLoading, setCreateServerLoading] = useState(false);
  const [file, setFile] = useState("");
  const user = useSelector(getSelf);
  const [name, setName] = useState<{ name: string; description: string }>({
    name: "",
    description: "",
  });
  const dispatch: any = useDispatch();

  function ChangeServerAvatar(e) {
    ...
    if (file) {
      ...
      reader.onload = function (ev) {
        setServerFileURL(ev.target.result);
      };
  ...
  async function HandleSubmitAddServer(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    // console.log(name);
    // return;
    const fileName = file
  ...
  return (
    <Dialog open={open} onClose={onClose}>
      ...
      >
        <div className="px-6">
          <div className="text-2xl font-bold text-[--header-primary] mt-4 text-center px-4">
            Thay đổi thông tin cá nhân
          </div>
          {/* <div className="text-[--header-secondary] mt-2 text-center">
            Tùy chỉnh máy chủ bằng cách đặt tên và thêm biểu tượng đại diện. Bạn
            có thể đổi bất cứ lúc nào.
          </div> */}
        </div>
        <div className="w-full mt-4 px-4">
          <div className="flex justify-center flex-row text-[--interactive-normal]">
            {!ServerFileURL ? (
              <svg
                width="80"
                height="80"
              ...
              <div>
                <img
                  src={ServerFileURL}
                  alt=""
                  className="w-20 h-20 rounded-full border-transparent bg-[--background-secondary-alt] "
          ...
          <div className="text-[--header-secondary] uppercase font-bold text-xs mt-4">
            server name
          </div>
          <div className="mt-2 bg-[--input-background] rounded-sm text-[--text-normal] h-10 p-[10px]">
            <input
              type="text"
              maxLength={100}
              name="name"
          ...
          <div className="text-[--text-muted] text-xs mt-2 mb-4"></div>
        </div>
        <div
          className="bg-[--modal-footer-background] p-4 w-full rounded-md flex flex-row-reverse justify-between items-center"
          style={{
            boxShadow: "inset 0 1px 0 hsl(var(--primary-630-hsl)/.6)",
          }}
        >
          <div>
            <button
              type="submit"
              value=""
              className="bg-[--brand-500] h-[38px] w-[96px] rounded-[3px] text-[--white-500] text-sm/4 hover:bg-[--brand-560] font-medium "
              disabled={CreateServerLoading}
            >
              <div>{!CreateServerLoading ? "Save" : "..."}</div>
            </button>
          </div>
        </div>
      </form>
